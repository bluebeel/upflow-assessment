...
import Table from '../components/TableWithVirtualization';
...
export function Virtualization() {
  ...
  React.useEffect(() => {
    if (router.isReady) {
      setParams({
        ...
        size: router.query.size ? Number(router.query.size) : 1000,
  ...
  const onSort = (sort: any) => {
    router.push(
      `/virtualization?column=${sort.id}&direction=${sort.desc ? 'DESC' : 'ASC'}&page=${
        params.page
      }&size=${params.size}`,
      undefined,
      { shallow: true }
  ...
  const onPagination = (page, pageSize) => {
    if (router.isReady) {
      router.push(
        `/virtualization?column=${params.column}&direction=${params.direction}&page=${page}&size=${pageSize}`,
        undefined,
        { shallow: true }
      );
    }
